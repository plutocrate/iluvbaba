...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (search) {
      params.push(`%${search}%`);
      // LIKE works on both SQLite and Postgres; ILIKE is Postgres-only
...
// GET /api/maps/mine - Get maps by current user
router.get('/mine', authMiddleware, async (req, res) => {
  const db = tryDb();
...
router.get('/:id', async (req, res) => {
  try {
    // Use subqueries to avoid column name clashes between tables in SQLite
    const r = await db().query(`
      SELECT m.id, m.title, m.map_json, m.upvotes, m.downvotes, m.created_at,
...
router.post('/', authMiddleware, async (req, res) => {
  ...
  try {
    // map_json may arrive as an object (from fetch JSON body) or a string — normalise both
    ...
    const mapStr = JSON.stringify(mapObj); // always store as clean JSON string
