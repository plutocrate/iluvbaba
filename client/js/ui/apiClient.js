/**
 * apiClient.js — All server communication
 */
...
class ApiClient {
  ...
  getUsername() { return localStorage.getItem('jwt_username') || ''; }
  ...
  async getMyMaps()   { return this.request('GET', '/maps/mine'); }
  async getMaps(sort = 'top', page = 1) { return this.request('GET', `/maps?sort=${sort}&page=${page}`); }
  async getMap(id)    { return this.request('GET', `/maps/${id}`); }
  async deleteMap(id) { return this.request('DELETE', `/maps/${id}`); }
  ...
  getLocalVote(mapId) { return localStorage.getItem(`vote_${mapId}`); }
}
